...
import { getTranslation, getInitialLanguage, Language } from './lib/translations';
...
export function isPostTrialLocked(user: UserProfile): boolean {
  ...
  // If user is actively subscribed, they are never locked out
  if (user.isSubscribed) return false;
  
  // IMMEDIATO APÓS O LOGIN/CRIAÇÃO DO MAPA:
  // Se o usuário já criou seu mapa e não é assinante, mostra imediatamente a tela Premium para testes de simulação.
  if (user.hasCreatedMap) {
    return true;
  }
  
  // If flagged for double-trials on same device
  if (user.trialUsed === true) {
    return true;
  }
  
  // If a trial end date is defined, check expiry (5 days)
  if (user.trialEndDate) {
    const trialEndMs = new Date(user.trialEndDate).getTime();
    if (!isNaN(trialEndMs)) {
      return Date.now() > trialEndMs;
    }
  }
  
  // Fallback to 5-day limit based on createdAt if trialEndDate is missing
  if (user.createdAt) {
    const createdAtMs = new Date(user.createdAt).getTime();
    if (!isNaN(createdAtMs)) {
      const trialDurationMs = 5 * 24 * 60 * 60 * 1000; // 5-day trial period
      return Date.now() > (createdAtMs + trialDurationMs);
    }
  }
...
export default function App() {
    ...
    try {
      ...
      if (firebaseUser && firebaseUser.email) {
        ...
        if (existingProfile) {
          ...
          const nameToUse = createMainName.trim() || firebaseUser.displayName || "Viajante Estelar";
  ...
  const handleRegisterAccountSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const nameToUse = createMainName || user.name || "Viajante Estelar";
  ...
  const handleLoginAccountSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (cloudUser) {
        ...
        targetUser = {
          userId: firebaseUser ? firebaseUser.uid : (cloudUser.userId || ""),
          name: cloudUser.name || "Viajante Estelar",
          ...
          profilePhoto: cloudUser.profilePhoto || "",
  ...
  useEffect(() => {
    if (isLoggedIn && loggedEmail) {
      if (user.scorePoints !== scorePoints) {
        const nextUser = { ...user, scorePoints };
        setUser(nextUser);
  ...
  useEffect(() => {
    if (user && user.scorePoints !== undefined && user.scorePoints !== scorePoints) {
      setScorePoints(user.scorePoints);
    }
  }, [user?.scorePoints]);
    ...
    const unsubProfile = subscribeToUserProfile(loggedEmail, (updatedProfile) => {
      if (updatedProfile) {
        setUser(prev => ({
          ...
          scorePoints: updatedProfile.scorePoints !== undefined ? updatedProfile.scorePoints : prev.scorePoints
        }));
      }
    }, (error) => {
  ...
  useEffect(() => {
    if (dailyMissions && dailyMissions.length > 0) {
      ...
      // Re-route dynamically to Cloud Firestore users/{uid}/missions/{today} if logged in
      const email = user.email || loggedEmail;
      if (isLoggedIn && email) {
        saveMissionToDatabase(email, todayStr, dailyMissions).catch(console.warn);
      }
    }
  }, [dailyMissions, user, isLoggedIn, loggedEmail]);
  ...
  const triggerGenerateMainMap = async (details: any) => {
    ...
    try {
      ...
      if (email) {
        // High-Precision subcollection lookup via currentChartId
        const activeChartId = user.currentChartId || details.currentChartId;
        if (activeChartId) {
          try {
            const dbChart = await loadNatalChartFromDatabase(email, activeChartId);
            if (dbChart && dbChart.mapData && dbChart.numerology && 
                dbChart.birthDate === details.birthDate &&
                dbChart.birthTime === details.birthTime &&
                dbChart.birthCity === details.birthCity) {
              console.log("[Astro DB] Natal chart loaded from users subcollection natalCharts.");
            ...
            console.warn("[Astro DB] Failed to load from subcollection natalCharts, fallback to cache/recalc:", dbErr);
          }
        }
      ...
      if (email && data.map && data.numerology) {
        ...
        try {
          // Direct subcollection persistence
  ...
  const handleUpdateUserProfile = (updatedDetails: Partial<UserProfile>) => {
    ...
    let nextChangesCount = user.mainMapChangesCount ?? 0;

    if (hasExistingMap && isBirthCoordinatesModified) {
      if (nextChangesCount >= 2) {
        triggerGlobalNotification(
          "Limite Atingido", 
          "Este Mapa Principal já foi alterado 2 vezes (o limite máximo permitido). Novas alterações não são permitidas.", 
          "error"
        );
        return;
      }
        ...
        <div id="premium-dashboard" className="relative z-10 pb-28">
          ...
          <header className="w-full bg-slate-900/60 border-b border-slate-850 px-4 py-4 sticky top-0 z-40 backdrop-blur-md">
            <div className="w-full max-w-7xl mx-auto flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                {/* Custom User avatar & profile metadata list - Clickable to update profile picture */}
                <div className="relative group cursor-pointer" title="Clique para alterar sua foto de perfil">
                  <input
                    type="file"
                    accept="image/*"
                  ...
                  <div 
                    onClick={() => {
                      document.getElementById('profile-avatar-top-file-input')?.click();
                    }}
                  ...
                  >
                    ...
                    <span className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity text-[8px] text-white font-bold font-sans uppercase">
                      Alterar
                    </span>
                  </div>
                ...
                <div>
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <h3 className="text-xs font-bold text-slate-205">{user.name || "Buscador Estelar"}</h3>
                  ...
                  <>
                    ...
                    {mapSubTab === 'criar_meu_mapa' && (
                      isLoggedIn && isPostTrialLocked(user) ? (
                        ...
                        {user.hasCreatedMap && (
                          <div className="bg-amber-500/5 p-5 border border-amber-500/20 rounded-2xl text-[11px] leading-relaxed text-slate-350 font-sans space-y-3">
                            ...
                            <div className="pt-2 border-t border-slate-800/80 flex justify-between items-center text-xs">
                              <span>Alterações realizadas do Mapa Principal:</span>
                              <span className="font-mono font-bold text-amber-500">{(user.mainMapChangesCount ?? 0)} / 2</span>
                            </div>
                            
                            {(user.mainMapChangesCount ?? 0) >= 2 ? (
                              <div className="p-3 bg-rose-500/10 border border-rose-500/20 rounded-xl text-rose-400 text-xs font-bold leading-relaxed">
                                ⚠️ Você já realizou 2 de 2 alterações permitidas no seu Mapa Principal. O limite vitalício total de alterações foi atingido e não é mais possível gerar um novo mapa.
                              </div>
                            ) : (
                              <div className="text-[10px] text-slate-400">
                                Restam <strong>{2 - (user.mainMapChangesCount ?? 0)}</strong> alterações. Após atingir este limite, a alteração das coordenadas celestes do Mapa Principal será bloqueada permanentemente.
                              </div>
                            )}
                          </div>
                        )}
                        ...
                        <form onSubmit={(e) => {
                          e.preventDefault();
                          if ((user.mainMapChangesCount ?? 0) >= 2 && user.hasCreatedMap) {
                            triggerGlobalNotification("Erro de Limite", "Você atingiu o limite de 2 alterações permitidas no Mapa Principal.", "error");
                            return;
                          }
                          ...
                          <fieldset disabled={(user.mainMapChangesCount ?? 0) >= 2 && user.hasCreatedMap} className="space-y-4">
                            <div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                            ...
                            <div className="md:col-span-5 bg-slate-900/40 p-5 rounded-3xl border border-slate-850 space-y-4 font-sans">
                              ...
                              <div className="p-3.5 bg-slate-950 rounded-2xl border border-slate-850 text-[10.5px] text-slate-400 space-y-1.5 leading-relaxed">
                                <span className="font-bold text-amber-500 text-[11px] block">⚠️ Política Cósmica de Mapas Extras:</span>
                                <ul className="list-disc pl-3.5 space-y-1 text-slate-400">
                                  <li><strong>Limite Vitalício Total:</strong> Cada usuário pode gerar no máximo <strong>2 Mapas Extras</strong> durante toda a vida útil da sua conta de acesso.</li>
                                  <li><strong>Regra de Exclusão Definitiva:</strong> Este limite de 2 criações é cumulativo. Se você deletar um Mapa Extra já cadastrado para liberar espaço, <strong>o limite não reabre nem zera</strong>. O slot excluído é considerado consumido definitivamente.</li>
                                  <li><strong>Seu status de criação:</strong> Você já criou <strong className="text-amber-400">{(user.extraMapsCreatedCount ?? 0)} de 2</strong> mapas extras vitalícios.</li>
                                </ul>
                              </div>
                              
                              {(user.extraMapsCreatedCount ?? 0) >= 2 ? (
                                <div className="p-4 bg-rose-950/20 border border-rose-500/20 rounded-2xl text-[11px] text-rose-400 leading-relaxed font-sans text-left space-y-2">
                                  <p className="font-bold">⚠️ Limite Vitalício Atingido</p>
                                  <p>Você já atingiu o limite cumulativo total de 2 criações de Mapas Extras ao longo da vida da conta. Não é possível registrar novos mapas de terceiros.</p>
                                </div>
                              ) : (
                                <form onSubmit={(e) => {
                                  e.preventDefault();
                                  ...
                                  triggerGlobalNotification(
                                    ...
                                    `O Mapa Astral de ${extraName} foi calculado e consumiu 1 do seu limite cumulativo de 2 mapas extras.`,
                                    "success"
                                  );
            ...
            {activeTab === 'configuracoes' && (
              <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in duration-300">
                ...
                <div className="space-y-6 animate-in fade-in duration-300">
                  ...
                  <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-800 space-y-4 font-sans">
                    ...
                    <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-850 text-xs space-y-1.5">
                      <div className="flex justify-between items-center text-xs">
                        <span className="font-semibold text-slate-300">Alterações do Mapa Principal:</span>
                        <span className="font-mono font-bold text-amber-500">{(user.mainMapChangesCount ?? 0)} / 2</span>
                      </div>
                      <p className="text-[10px] text-slate-400 leading-relaxed">
                        {(user.mainMapChangesCount ?? 0) >= 2 ? (
                          <span className="text-rose-450 font-bold">⚠️ Limite vitalício atingido. Não é mais possível alterar as coordenadas celestes do seu Mapa Principal.</span>
                        ) : (
                          <span>Você possui <strong>{2 - (user.mainMapChangesCount ?? 0)}</strong> alterações restantes para o Mapa Principal.</span>
                        )}
                      </p>
                    </div>
                  ...
                  <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-800 space-y-4 font-sans">
                    <h3 className="text-xs font-bold font-mono text-slate-400 uppercase tracking-widest pb-2 border-b border-slate-850">Acessibilidade</h3>
                    
                    <div className="flex justify-between items-center text-xs">
                      <div>
                        <strong>Modo de Alto Contraste</strong>
                        <p className="text-[10px] text-slate-500">Aumenta o contraste de textos, botões e bordas para garantir melhor visibilidade.</p>
                      </div>
                  ...
                  {/* Limpar Cache Panel */}
                  <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-800 space-y-4 font-sans">
                    <h3 className="text-xs font-bold font-mono text-slate-400 uppercase tracking-widest pb-2 border-b border-slate-850">Desempenho e Armazenamento</h3>
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center text-xs gap-3">
                      <div>
                        <strong className="text-slate-200">Limpar Cache do Sistema</strong>
                        <p className="text-[10px] text-slate-500 mt-1">Apaga os arquivos temporários e caches de desempenho de relatórios. Não afeta seus mapas nem sua conta de acesso.</p>
                      </div>
                      <button 
                  ...
                  <div className="p-4 bg-slate-950 rounded-2xl border border-slate-850 text-[10.5px] text-slate-550 leading-relaxed font-mono">
                    Em caso de dúvidas, faça contato com a equipe de suporte pelo canal oficial de auditorias de integridade celestial.
                  </div>
                ...
                {showDeleteConfirm && (
                  <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-slate-900 border border-rose-500/30 p-6 rounded-3xl max-w-sm w-full text-center space-y-4 shadow-[0_20px_50px_rgba(0,0,0,0.85)] animate-in fade-in zoom-in-95 duration-200">
                      ...
                      <h3 className="text-sm font-bold text-slate-100 font-sans">Excluir sua conta?</h3>
                      <p className="text-xs text-slate-400 font-sans leading-relaxed">
                        Você deseja excluir sua conta? Ao excluir sua conta todos os seus dados mapas registros estatísticas serão excluídos da plataforma.
                      </p>
                      <div className="flex flex-col gap-2 pt-2">
                        <button
